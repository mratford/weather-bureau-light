"""Tests for the HTTP client, including authentication, caching, retries, and fallback."""
...
def test_expired_cache_refetches(config):
    ...
    client = make_client(config, handler)
    client.get("/x")
    client.get("/x", ttl=0)  # Treat this entry as immediately stale.
    assert len(calls) == 2
...
def test_stale_cache_is_served_when_the_api_fails(config, monkeypatch):
    """A forecast a few hours old is preferable to an error page."""
    monkeypatch.setattr(time, "sleep", lambda s: None)
    state = {"fail": False}
...
def test_ensure_base_url_falls_back_to_a_version_the_key_accepts(config):
    """A key may be subscribed to only one live service version."""
...
# --- Hour-aligned cache expiry --------------------------------------------------
#
# The forecast advances on the hour and its leading columns then disappear. Forecast
# and instance responses therefore expire at that boundary rather than after a fixed
# interval.


def at(hour: float) -> float:
    """Return an epoch timestamp at a given offset from an hour boundary."""
...
def write_at(cache: DiskCache, key: str, value, when: float) -> None:
    """Seed the cache with a specified write time."""
...
def test_cache_is_fresh_within_the_same_hour(tmp_path, monkeypatch):
    cache = DiskCache(tmp_path)
    # Written at 09:06 and read at 09:50: same hour, still fresh.
...
def test_cache_expires_once_the_hour_turns(tmp_path, monkeypatch):
    cache = DiskCache(tmp_path)
    # 09:06 to 10:01 crosses the boundary.
    write_at(cache, "k", {"v": 1}, at(9.10))
...
def test_floor_prevents_thrash_at_the_boundary(tmp_path, monkeypatch):
    """A fetch at 09:59 remains valid one minute later."""
    cache = DiskCache(tmp_path)
...
def test_floor_does_not_hold_data_past_its_hour(tmp_path, monkeypatch):
    cache = DiskCache(tmp_path)
    write_at(cache, "k", {"v": 1}, at(9.99))
    # After the five-minute minimum age, the boundary determines expiry.
...
def test_forecast_requests_are_hour_aligned(config, monkeypatch):
    """Two fetches in the same hour use one API request."""
    calls = []
...
#
# The stale fallback keeps the page useful during an outage. These tests also check
# that callers can identify when it was used.
...
def test_stale_fallback_is_marked_and_keeps_the_original_time(config, monkeypatch):
    """Reported age comes from the cached data, not the failed retry."""
    monkeypatch.setattr(time, "sleep", lambda s: None)
    state = {"fail": False}
...
def test_cache_hit_reports_when_the_data_was_stored(config, monkeypatch):
    """A cache hit is current for the request but older than the current time."""
    client = make_client(config, lambda r: httpx.Response(200, json={"good": True}))
    monkeypatch.setattr(time, "time", lambda: at(9.0))
    client.fetch("/x")
...
def test_health_state_records_failure_even_when_stale_data_rescues_the_page(config, monkeypatch):
    """The page recovers while recording that the API refused the request."""
    monkeypatch.setattr(time, "sleep", lambda s: None)
    state = {"fail": False}
